...
parser.add_argument('--trained_model',
                    default='weights/ssd300_COCO_2000.pth', type=str,
...
# first load gt
    ...
    # read list of images
    with open(imagesetfile, 'r') as f: # test.txt
        lines = f.readlines()
    ...
    for imagename in imagenames:
        R = [obj for obj in recs[imagename] if obj['name'] == classname] # R is xml_info cressponding to classname
...
def test_net(save_folder, net, cuda, dataset, transform, top_k,
    ...
    for i in range(num_images):
        ...
        for j in range(1, detections.size(1)):
            dets = detections[0, j, :] #　N * 5 array
            # print("gt(0.):", dets[:, 0].gt(0.).expand(5, dets.size(0)).t().size())
            mask = dets[:, 0].gt(0.).expand(5, dets.size(0)).t() # .gt() 比较前者张量是否大于后者 .t() means transpose
            ...
            # print('all_boxes.size: ', np.shape(all_boxes)) #(4, 5500)
